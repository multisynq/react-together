...
import fs from 'fs'
import { ConfigEnv, Plugin, UserConfig, UserConfigExport, UserConfigFn, defineConfig as viteDefineConfig } from 'vite'
...
export async function defineConfig(userConfigExport: UserConfigExport): Promise<UserConfigFn> {
  ...
  const userConfigOrFn = await userConfigExport
